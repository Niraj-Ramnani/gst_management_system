...
export default function UploadPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 sm:space-y-8 animate-in">
      <div>
        <h1 className={clsx("section-title text-xl sm:text-2xl", isLight ? "text-[#0f172a]" : "text-white")}>Upload Invoice</h1>
        <p className={clsx("text-xs sm:text-sm mt-1", isLight ? "text-slate-500" : "text-slate-400")}>Upload a PDF or image. Our AI will extract all GST fields automatically.</p>
      </div>
        ...
        <div className="card p-5 sm:p-8 space-y-6 sm:space-y-8">
              ...
              {['purchase', 'sale'].map(type => (
                <button
                  ...
                  className={clsx(
                    ...
                    invoiceType === type
                      ? (isLight ? 'border-primary-500 bg-primary-50 text-primary-600' : 'border-primary-500 bg-primary-900/30 text-primary-400')
                      : (isLight ? 'border-slate-200 bg-slate-50/50 text-slate-500 hover:border-slate-300' : 'border-slate-800 bg-slate-900/40 text-slate-500 hover:border-slate-700')
                  )}
                >
                  {type}
                </button>
          ...
          <div
            ...
            className={clsx(
              ...
              dragging 
                ? (isLight ? 'border-primary-500 bg-primary-50' : 'border-primary-500 bg-primary-900/20') 
                : (isLight ? 'border-slate-200 hover:border-primary-300 hover:bg-primary-50/30' : 'border-slate-800 hover:border-slate-700 hover:bg-white/[0.01]')
            )}
            ...
            {file ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="min-w-0">
                  <p className={clsx("font-bold text-sm sm:text-base truncate max-w-[200px] sm:max-w-md", isLight ? "text-[#0f172a]" : "text-white")}>{file.name}</p>
                  <p className={clsx("text-xs mt-1 font-mono uppercase", isLight ? "text-slate-400" : "text-slate-500")}>{formatFileSize(file.size)}</p>
                </div>
                <button
              ...
              <div className="flex flex-col items-center gap-4">
                <div className={clsx("w-16 h-16 rounded-2xl flex items-center justify-center border transition-colors", isLight ? "bg-slate-50 border-slate-200 group-hover:border-primary-400" : "bg-slate-900 border-slate-800 group-hover:border-primary-500/30")}>
                  <Upload size={32} className={clsx("transition-colors", isLight ? "text-slate-400 group-hover:text-primary-500" : "text-slate-600 group-hover:text-primary-400")} />
                </div>
                <div>
                  <p className={clsx("font-bold text-sm sm:text-base", isLight ? "text-slate-600" : "text-slate-300")}>Drag & drop your invoice here</p>
                  <p className={clsx("text-xs sm:text-sm mt-1", isLight ? "text-slate-400" : "text-slate-500")}>or click to browse your files</p>
                </div>
          ...
          <div className={clsx("rounded-2xl p-5 border backdrop-blur-sm", isLight ? "bg-slate-50 border-slate-200" : "bg-slate-950/40 border-slate-800/60")}>
            <p className={clsx("text-[10px] font-black uppercase tracking-widest mb-4 sm:mb-5", isLight ? "text-slate-400" : "text-slate-500")}>Next Steps:</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
              {[
                ...
                <div key={step.title} className="flex items-start gap-3">
                  ...
                  <div>
                    <p className={clsx("text-[10px] font-bold uppercase leading-none mb-1", isLight ? "text-slate-600" : "text-slate-300")}>{step.title}</p>
                    <p className={clsx("text-[10px]", isLight ? "text-slate-400" : "text-slate-500")}>{step.desc}</p>
                  </div>
                </div>
              ))}
            </div>
